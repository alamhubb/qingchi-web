...
import com.qingchi.server.model.UserImgVO;
...
public class IdentityController {
    ...
    @PostMapping("auth")
    public ResultVO<?> identityAuth(UserDO user, @RequestBody @Valid UserImgVO img) {
        //第一版，只支持机器认证，3次每人只能认证4次,3次系统审核1次人工审核，0,1,2,3最大数是3
        if (user.getAuthNum() < 3) {
            IdentityImgDO identityImgDO = img.toIdentityImgDO(user, imgUrl);
            ...
            if (authFlag) {
                //前台提示认证成功，
                return new ResultVO<>(new UserDetailVO(UserUtils.get(identityImgDO.getUserId()), true));
            } else {
                return new ResultVO<>("认证失败，请在个人信息照片墙中上传本人清晰露脸的真实照片后再进行认证");
            }
            //            if (user.getAuthNum() < 3) {
            /*} else {
                //改为审核中，由人工审核
            ...
            return new ResultVO<>("已达到审认证次数上线，" + ErrorMsg.CONTACT_SERVICE);
        }
    }
}
